 ...
 *      Author: Amrita-SDN
 *
 *  This is a test-dpi plugin, to demonstrate the interfacing capablity of DPI with OVS
 *  This plugin gets every packet from OVS and just log the headers in a log file.
 *
 *  This can be extended to full-fledged DPI engine as we now have raw ethernet packet from
 *  OVS.
 */
...
int32 engine_init()
{
    ...
    if(logfile==NULL)
    {
    //	DpiWriteLog(DPIERR,"Unable to create log.txt file.");
        return -1;
    }
  //  DpiWriteLog(DPIINFO,"Starting kspviswa engine");
